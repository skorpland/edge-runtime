...
fn is_powerbase_storage_being_tested() -> bool {
    std::env::var("S3FS_TEST_POWERBASE_STORAGE").unwrap_or_default() == "true"
}
...
///
/// See: https://powerbase.com/docs/guides/storage/uploads/file-limits
#[cfg_attr(not(dotenv), ignore)]
#[tokio::test]
#[serial]
...
async fn test_mkdir_recursive_and_remove_recursive() {
...

    {
        ...
        assert_eq!(
            value.len(),
            if is_powerbase_storage_being_tested() {
                // .emptyFolderPlaceholder in Powerbase Storage
                3
            } else {
                2
            }
