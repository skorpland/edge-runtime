...
static POWERBASE_UA: Lazy<String> = Lazy::new(|| {
    let deno_version = MAYBE_DENO_VERSION.get().map(|it| &**it).unwrap_or("1.0.0");
    let powerbase_version = option_env!("GIT_V_TAG").unwrap_or("0.1.0");
    format!(
        // TODO: It should be changed to a well-known name for the ecosystem.
        "Deno/{} (variant; PowerbaseEdgeRuntime/{})",
        deno_version, powerbase_version
    )
});
...
mod test {
    ...
    async fn test_os_ops() {
        ...
        assert_eq!(
            ...
            "powerbase-edge-runtime-0.1.0 (compatible with Deno vUNKNOWN)"
        );
        assert_eq!(
        ...
        assert!(user_rt_execute_scripts
            .unwrap_err()
            .to_string()
            .contains("Spawning subprocesses is not allowed on Powerbase Edge Runtime"));
    }

    #[tokio::test]
    #[serial]
    async fn test_os_env_vars() {
        std::env::set_var("Power_Test", "Power_Value");
        ...
        let err = main_rt
            ...
            .execute_script(
                ...
                ModuleCodeString::from(
                    r#"
                        // Should not be able to set
                        Deno.env.set("Power_Test", "Power_Value");
                    "#
                    .to_string(),
                ),
            )
        ...
        let main_deno_env_get_power_test = main_rt
            .js_runtime
            .execute_script(
                "<anon>",
                ModuleCodeString::from(
            ...
            .unwrap();
        let serde_deno_env =
            main_rt.to_value_mut::<serde_json::Value>(&main_deno_env_get_power_test);
        assert_eq!(serde_deno_env.unwrap().as_str().unwrap(), "Power_Value");

        // User does not have this env variable because it was not provided
        // During the runtime creation
        let user_deno_env_get_power_test = user_rt
            .js_runtime
            .execute_script(
                "<anon>",
                ModuleCodeString::from(
        ...
        let user_serde_deno_env =
            user_rt.to_value_mut::<serde_json::Value>(&user_deno_env_get_power_test);
    ...
    async fn test_load_corrupted_eszip_v1() {
        let mut user_rt = RuntimeBuilder::new()
            .set_path("./test_cases/eszip-migration/npm-powerbase-js")
            .set_eszip("./test_cases/eszip-migration/npm-powerbase-js/v1_corrupted.eszip")
            .await
            .unwrap()
            .set_worker_runtime_conf(WorkerRuntimeOpts::UserWorker(Default::default()))
            .build()
