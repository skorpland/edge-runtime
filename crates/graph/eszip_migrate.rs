...
mod v0 {
    ...
    use eszip_async_trait::{AsyncEszipDataRead, POWERBASE_ESZIP_VERSION_KEY, VFS_ESZIP_KEY};
    ...
    pub async fn try_migrate_v0_v1(
        ...
        v1_eszip.add_opaque_data(
            String::from(POWERBASE_ESZIP_VERSION_KEY),
            Arc::from(b"1" as &[u8]),
        );
        ...
        static BLOCKLIST: Lazy<HashSet<&str>> =
            Lazy::new(|| HashSet::from([POWERBASE_ESZIP_VERSION_KEY, VFS_ESZIP_KEY]));
...
mod test {
    ...
    async fn test_extract_v0() {
        test_extract_eszip(
            PathBuf::from(format!("{}/npm-powerbase-js/index.ts", MIGRATE_TEST_DIR)),
            PathBuf::from(format!("{}/npm-powerbase-js/v0.eszip", MIGRATE_TEST_DIR)),
        )
        .await;
    }
    ...
    async fn test_extract_v1() {
        test_extract_eszip(
            PathBuf::from(format!("{}/npm-powerbase-js/index.ts", MIGRATE_TEST_DIR)),
            PathBuf::from(format!("{}/npm-powerbase-js/v1.eszip", MIGRATE_TEST_DIR)),
        )
        .await;
    }
    ...
    async fn test_extract_v0_corrupted() {
        test_extract_eszip(
            PathBuf::from(format!("{}/npm-powerbase-js/index.ts", MIGRATE_TEST_DIR)),
            PathBuf::from(format!(
                "{}/npm-powerbase-js/v0_corrupted.eszip",
                MIGRATE_TEST_DIR
            )),
        )
        .await;
    ...
    async fn test_extract_v1_corrupted() {
        test_extract_eszip(
            PathBuf::from(format!("{}/npm-powerbase-js/index.ts", MIGRATE_TEST_DIR)),
            PathBuf::from(format!(
                "{}/npm-powerbase-js/v1_corrupted.eszip",
                MIGRATE_TEST_DIR
            )),
        )
        .await;
    ...
    async fn test_vfs_registry_migration_v1_1_xx_hash3() {
        test_vfs_npm_registry_migration_1_45_x(
            fs::read(PathBuf::from(format!(
                "{}/npm-powerbase-js/v1_1_xx_hash3.eszip",
                MIGRATE_TEST_DIR
            )))
            .await
            .unwrap(),
    ...
    async fn test_vfs_registry_migration_v1_1_no_checksum() {
        test_vfs_npm_registry_migration_1_45_x(
            fs::read(PathBuf::from(format!(
                "{}/npm-powerbase-js/v1_1_no_checksum.eszip",
                MIGRATE_TEST_DIR
            )))
            .await
            .unwrap(),
