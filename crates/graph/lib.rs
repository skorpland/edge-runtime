...
use eszip_async_trait::{
    ...
    POWERBASE_ESZIP_VERSION, POWERBASE_ESZIP_VERSION_KEY, VFS_ESZIP_KEY,
};
...
impl LazyLoadableEszip {
    ...
    pub async fn ensure_version(&self) -> Result<(), anyhow::Error> {
        let version = OptionFuture::<_>::from(
            self.ensure_module(POWERBASE_ESZIP_VERSION_KEY)
                .map(|it| async move { it.source().await }),
        )
        .await
        .flatten();

        if !matches!(version, Some(ref v) if v.as_ref() == POWERBASE_ESZIP_VERSION) {
            bail!(EszipError::UnsupportedVersion {
                expected: POWERBASE_ESZIP_VERSION,
    ...
    P: AsRef<Path>,
{
    ...
    eszip.add_opaque_data(
        ...
        Arc::from(POWERBASE_ESZIP_VERSION),
    );
