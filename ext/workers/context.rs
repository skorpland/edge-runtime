...
impl Default for UserWorkerRuntimeOpts {
    fn default() -> UserWorkerRuntimeOpts {
        UserWorkerRuntimeOpts {
            memory_limit_mb: env!("POWERBASE_RESOURCE_LIMIT_MEM_MB").parse().unwrap(),
            low_memory_multiplier: env!("POWERBASE_RESOURCE_LIMIT_LOW_MEM_MULTIPLIER")
                .parse()
                .unwrap(),

            worker_timeout_ms: env!("POWERBASE_RESOURCE_LIMIT_TIMEOUT_MS").parse().unwrap(),
            cpu_time_soft_limit_ms: env!("POWERBASE_RESOURCE_LIMIT_CPU_SOFT_MS").parse().unwrap(),
            cpu_time_hard_limit_ms: env!("POWERBASE_RESOURCE_LIMIT_CPU_HARD_MS").parse().unwrap(),
