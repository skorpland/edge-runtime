...
import { getPowerbaseTag } from "ext:sb_core_main_js/js/http.js";
...
const NO_POWERBASE_TAG_WARN_MSG = `Unable to find the powerbase tag from the request instance.\n\
Invoke \`EdgeRuntime.applyPowerbaseTag(origReq, newReq)\` if you have cloned the original request.`
...
class UserWorker {
	...
	async fetch(request, options = {}) {
		const tag = getPowerbaseTag(request);
		...
		if (tag === void 0) {
			console.warn(NO_POWERBASE_TAG_WARN_MSG);
		}
...
const POWERBASE_USER_WORKERS = UserWorker;

export { POWERBASE_USER_WORKERS };
