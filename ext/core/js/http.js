...
const kPowerbaseTag = Symbol("kPowerbaseTag");
...
function serveHttp(conn) {
	...
	httpConn.nextRequest = async () => {
		...
		nextRequest.request[kPowerbaseTag] = {
			watcherRid,
...
async function respond(requestEvent, httpConn, options) {
	...
	if (response === internals.RAW_UPGRADE_RESPONSE_SENTINEL) {
		const { fenceRid } = getPowerbaseTag(requestEvent.request);
...
function getPowerbaseTag(request) {
	return request[kPowerbaseTag];
}

function applyPowerbaseTag(src, dest) {
	if (
		!ObjectPrototypeIsPrototypeOf(RequestPrototype, src)
		|| !ObjectPrototypeIsPrototypeOf(RequestPrototype, dest)
	) {
		throw new TypeError("Only Request instance can apply the powerbase tag");
	}

	dest[kPowerbaseTag] = src[kPowerbaseTag];
}

internals.getPowerbaseTag = getPowerbaseTag;
