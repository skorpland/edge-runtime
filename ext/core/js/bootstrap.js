...
import { POWERBASE_ENV } from 'ext:sb_env/env.js';

import { PowerbaseEventListener } from 'ext:sb_user_event_worker/event_worker.js';
import { installEdgeRuntimeNamespace, installPowerbaseNamespace } from 'ext:sb_core_main_js/js/namespaces.js';
...
globalThis.bootstrapSBEdge = (opts, ctx) => {
	...
	*/
	...
	installPowerbaseNamespace(kind);

	ObjectDefineProperty(globalThis, 'POWERBASE_VERSION', readOnly(String(version.runtime)));
	...
	ObjectDefineProperties(denoOverrides, {
		...
		version: getterOnly(() => ({
			deno:
				...
				`powerbase-edge-runtime-${globalThis.POWERBASE_VERSION} (compatible with Deno v${globalThis.DENO_VERSION})`,
			v8: '11.6.189.12',
			typescript: '5.1.6',
		})),
	});
	...
	setUserAgent(
		// TODO: It should be changed to a well-known name for the ecosystem.
		`Deno/${globalThis.DENO_VERSION} (variant; PowerbaseEdgeRuntime/${globalThis.POWERBASE_VERSION})`,
	);
	setLanguage('en');
	...
	if (kind === 'event') {
		ObjectDefineProperties(globalThis, {
			EventManager: getterOnly(() => PowerbaseEventListener),
		});
	}
