...
import { POWERBASE_USER_WORKERS } from "ext:sb_user_workers/user_workers.js";
import { applyPowerbaseTag } from "ext:sb_core_main_js/js/http.js";
...
function installEdgeRuntimeNamespace(kind, terminationRequestTokenRid) {
	...
	switch (kind) {
		case "main":
			props = {
				ai: MAIN_WORKER_API,
				userWorkers: POWERBASE_USER_WORKERS,
				getRuntimeMetrics: () => /* async */ ops.op_runtime_metrics(),
				applyPowerbaseTag: (src, dest) => applyPowerbaseTag(src, dest),
 ...
 * @param {"user" | "main" | "event"} kind 
 */
function installPowerbaseNamespace(kind) {
  const props = {
    ai: USER_WORKER_API
  };

  ObjectDefineProperty(globalThis, "Powerbase", {
    get() {
      return props;
    },
    configurable: true,
