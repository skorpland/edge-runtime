...
Deno.serve(async () => {
  try {
    ...
    if (deno_region) {
      ...
      await redis.hincrby('power-edge-counter', deno_region, 1)
    } else {
      // Increment localhost counter
      await redis.hincrby('power-edge-counter', 'localhost', 1)
    }

    // Get all values
    const counterHash: Record<string, number> | null = await redis.hgetall('power-edge-counter')
